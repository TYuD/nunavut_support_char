...
static inline float get_nan() { const uint_fast32_t MaxInt32 = 0x7FFFFFFF; return *((const float*)(&MaxInt32)); }
#define  Nan32   get_nan()
static const float Huge32   = 3.402823466e+38F;
...
{
    TEST_ASSERT_EQUAL_UINT32(CHAR_BIT*4-0, nunavutSaturateBufferFragmentBitLength(4, 0           , CHAR_BIT*4));
    TEST_ASSERT_EQUAL_UINT32(CHAR_BIT*4-1, nunavutSaturateBufferFragmentBitLength(4, 1           , CHAR_BIT*4));
    TEST_ASSERT_EQUAL_UINT32(CHAR_BIT*2-0, nunavutSaturateBufferFragmentBitLength(4, 0           , CHAR_BIT*2));
    TEST_ASSERT_EQUAL_UINT32(CHAR_BIT*2-1, nunavutSaturateBufferFragmentBitLength(4, CHAR_BIT*2+1, CHAR_BIT*3));
    TEST_ASSERT_EQUAL_UINT32(CHAR_BIT*0-0, nunavutSaturateBufferFragmentBitLength(2, CHAR_BIT*3  , CHAR_BIT*3));
}
...
static inline uint_fast8_t helperHalfcharsCombine(const uint_fast8_t* src)
{
...
{
    const uint_fast8_t src[] = { x11, x22, x33, x44, x55, x66, x77, x88, x99, xAA, xBB, xCC, xDD, xEE, xFF };
    uint_fast8_t dst[6];
    ...
    nunavutGetBits(dst, src, 0, 0, 4*CHAR_BIT);
    ...
    nunavutGetBits(dst, src, 6, 6*CHAR_BIT, 4*CHAR_BIT);
    TEST_ASSERT_EQUAL_HEX8(x00, dst[0]);   // all bytes zero-extended
    TEST_ASSERT_EQUAL_HEX8(x00, dst[1]);
    TEST_ASSERT_EQUAL_HEX8(x00, dst[2]);
    TEST_ASSERT_EQUAL_HEX8(x00, dst[3]);
    ...
    memset(dst, xAA, sizeof(dst));
    nunavutGetBits(dst, src, 6, 5*CHAR_BIT, 4*CHAR_BIT);
    ...
    memset(dst, xAA, sizeof(dst));
    nunavutGetBits(dst, src, 6, 5*CHAR_BIT - (CHAR_BIT >> 1), 4*CHAR_BIT);
    ...
    nunavutGetBits(dst, src, 7, (CHAR_BIT >> 1), 4*CHAR_BIT);
    ...
    nunavutGetBits(dst, src, 7, (CHAR_BIT >> 1), 4*CHAR_BIT - (CHAR_BIT >> 1));
...
{
    int64_t i64 = 0;
    nunavutSetIxx(&i64, sizeof(i64), 0, -1, 64);
...
{
    int64_t i64 = 0;
    nunavutSetIxx(&i64, sizeof(i64), 0, -255, 64);
    TEST_ASSERT_EQUAL_HEX64(((int64_t)(-255)), i64);
}

static void testNunavutSetIxx_neg255_tooSmall(void)
{
    int64_t i64 = 0;
    nunavutSetIxx(&i64, sizeof(i64), 0, -255, 8);
    TEST_ASSERT_EQUAL_HEX64(0x01, i64);
}

static void testNunavutSetIxx_bufferOverflow(void)
{
    uint_fast8_t buffer[] = {x00, x00, x00};
    int rc = nunavutSetIxx(buffer, 3, 2*CHAR_BIT, xAA, CHAR_BIT);
    TEST_ASSERT_EQUAL_INT8(NUNAVUT_SUCCESS, rc);
    TEST_ASSERT_EQUAL_HEX8(xAA, buffer[2]);
    rc = nunavutSetIxx(buffer, 2, 2*CHAR_BIT, x00, CHAR_BIT);
...
{
    uint_fast8_t buffer[] = {0x00, 0x00};
    int_fast8_t rc = nunavutSetBit(buffer, 1, CHAR_BIT, true);
    TEST_ASSERT_EQUAL_INT8(-NUNAVUT_ERROR_SERIALIZATION_BUFFER_TOO_SMALL, rc);
...
{
    const uint_fast16_t data[] = {0xAAFE, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
    TEST_ASSERT_EQUAL_HEX8(0xFE, nunavutGetU8(data, sizeof(data), 0, 8U));
}

static void testNunavutGetU8_tooSmall(void)
{
    const uint_fast16_t data[] = {0xAAFF, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
...
{
    const uint_fast8_t data[] = {xAA, xAA, xAA, xAA, xAA, xAA, xAA, xAA};
...
{
    uint_fast16_t packed_float = nunavutFloat16Pack(Huge32);
    ...
    packed_float = nunavutFloat16Pack(-Huge32);
    TEST_ASSERT_EQUAL_HEX16_MESSAGE(0x0000, (0x03FF & packed_float), "Mantessa bits were not 0 for -INFINITY.");
    TEST_ASSERT_EQUAL_HEX16_MESSAGE(0x7C00, (0x7C00 & packed_float), "-INFINITY did not set bits G5 - G4+w");
...
static void helperPackUnpack(float source_value, uint_fast16_t compare_mask, unsigned iterations)
{
...
#pragma GCC diagnostic pop
    helperPackUnpack( Huge32, 0xFF00, 10);
    helperPackUnpack(-Huge32, 0xFF00, 10);
}
...
{
    uint_fast32_t x = 0;
    nunavutSetF16(&x, sizeof(x), 0, 3.14f);
...
static void testNunavutGetF16(void)
{
    // >>> hex(int.from_bytes(np.array([np.float16('3.14')]).tobytes(), 'little'))
    // '0x4248'
    const uint_fast32_t x = 0x4248;
    const float result = nunavutGetF16(&x, sizeof(x), 0);
 ...
 */
static void helperAssertSerFloat32SameAsIEEE(const float original_value, const void* serialized_result)
{
    typedef union
    {
        float f;
...
{
    uint_fast8_t buffer[] = {x00, x00, x00, x00};
    ...
    memset(buffer, 0, sizeof(buffer));
    nunavutSetF32(buffer, sizeof(buffer), 0, -Nan32);
    ...
    memset(buffer, 0, sizeof(buffer));
    nunavutSetF32(buffer, sizeof(buffer), 0, Huge32);
    helperAssertSerFloat32SameAsIEEE(Huge32, buffer);
...
{
    double x = 0;
    nunavutSetF64(&x, sizeof(x), 0, 3.141592653589793);
    ...
    memset(&x, 0, sizeof(x));
    nunavutSetF64(&x, sizeof(x), 0, -Nan32);
    helperAssertSerFloat64SameAsIEEE(-Nan32, &x);
    ...
    memset(&x, 0, sizeof(x));
    nunavutSetF64(&x, sizeof(x), 0, Huge32);
    helperAssertSerFloat64SameAsIEEE(Huge32, &x);
