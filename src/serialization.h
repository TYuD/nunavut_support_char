...
static inline size_t nunavutSaturateBufferFragmentBitLength(
    const size_t buffer_size_chars, const size_t fragment_offset_bits, const size_t fragment_length_bits)
{
    const size_t size_bits = (size_t)(buffer_size_chars * CHAR_BIT);
...
/// DSDL bit-level serialization specification. The offsets may be arbitrary (may exceed CHAR_BIT bits).
...
/// Byte is the same as char and may be 8, 16, 32, ... bits of length
/// Arguments:
///     dst             Destination buffer. Shall be at least ceil(length_bits/CHAR_BIT) bytes large.
///     dst_offset_bits Offset in bits from the destination pointer. May exceed CHAR_BIT.
///     length_bits     The number of bits to copy. Both source and destination shall be large enough.
///     src             Source buffer. Shall be at least ceil(length_bits/CHAR_BIT) bytes large.
///     src_offset_bits Offset in bits from the source pointer. May exceed CHAR_BIT.
                                   ...
                                   const size_t src_offset_bits)
{
    ...
    if ((0U == (src_offset_bits % CHAR_SHIFT_MASK)) && (0U == (dst_offset_bits % CHAR_SHIFT_MASK)))  // Aligned copy, optimized, most common case.
    {
        const char* last_src;
        char*       last_dst;
        unsigned    mask, length_mod;
        const size_t length_chars = (size_t)(length_bits >> CHAR_SHIFT);
        (void)memmove(pdst, psrc, length_chars);
        length_mod = length_bits & CHAR_SHIFT_MASK;
        if (length_mod == 0)
            return;
        last_src  = psrc + length_chars;
        last_dst  = pdst + length_chars;
        mask      = (unsigned)((1U << length_mod) - 1U);
        *last_dst = (*last_dst & (unsigned)(~mask)) | (*last_src & mask);
        return;
    }
    #if ((-1) >> 1) < 0  // Right shift is arithmetic (the most common case). So, the fast algorithm is allowed
    {
    ...
    #else  // The below algorithm is not adopted for CHAR_BIT
    {
...
/// It extracts (len_bits) bits that are offset by (off_bits) from the origin of (buf) whose size is (buf_size_chars).
/// If the requested (len_bits+off_bits) overruns the buffer, the missing bits are implicitly zero-extended.
/// If (len_bits % CHAR_BIT != 0), the output buffer is right-zero-padded up to the next byte boundary.
/// If (off_bits % CHAR_BIT == 0), the operation is delegated to memmove(); otherwise, a much slower unaligned bit copy
/// algorithm is employed. See @ref nunavutCopyBits() for further details.
static inline void nunavutGetBits(void* const output,
                                  const void* const buf,
                                  const size_t buf_size_chars,
                                  ...
                                  const size_t len_bits)
{
    ...
    // The former case ensures that if we're copying < CHAR_BIT bits, the MSB in the destination will be zeroed out.
...
///     buf_size_chars  Size of the above, in bytes.
...
static inline int nunavutSetBit(
    void* const buf,
    const size_t buf_size_chars,
    const size_t off_bits,
    const bool value)
...
static inline int nunavutSetUxx(
    void* const buf,
    const size_t buf_size_chars,
    const size_t off_bits,
    ...
    const size_t len_bits)
{
    static_assert(64U == (sizeof(uint64_t) * CHAR_BIT), "Unexpected size of uint64_t");
    NUNAVUT_ASSERT(buf != NULL);
...
static inline int nunavutSetIxx(
    void* const buf,
    const size_t buf_size_chars,
    const size_t off_bits,
    const int64_t value,
...
///     buf_size_chars  The size of the source buffer, in bytes (chars). Reads past this limit will return zero bits.
///     off_bits        Offset, in bits, from the beginning of the buffer. May exceed one byte.
///     len_bits        Length of the serialized representation, in bits. Zero returns 0. Out-of-range values saturated.

static inline uint_fast8_t nunavutGetU8(const void* const buf,
                                        const size_t buf_size_chars,
                                        const size_t off_bits,
                                        const size_t len_bits)
{
    ...
    NUNAVUT_ASSERT(bits <= 8U);
    NUNAVUT_ASSERT(buf != NULL);
    nunavutCopyBits(&val, 0U, bits, buf, off_bits);
    return val;
}

static inline bool nunavutGetBit(const void* const buf,
                                 const size_t buf_size_chars,
                                 const size_t off_bits)
{
...
static inline uint_fast16_t nunavutGetU16(const void* const buf,
                                          const size_t buf_size_chars,
                                          const size_t off_bits,
                                          const size_t len_bits)
{
    ...
    NUNAVUT_ASSERT(bits <= 16U);
    NUNAVUT_ASSERT(buf != NULL);
    nunavutCopyBits(&val, 0U, bits, buf, off_bits);
    return val;
}

static inline uint_fast32_t nunavutGetU32(const void* const buf,
                                          const size_t buf_size_chars,
                                          const size_t off_bits,
                                          const size_t len_bits)
{
    ...
    NUNAVUT_ASSERT(bits <= 32U);
    NUNAVUT_ASSERT(buf != NULL);
    nunavutCopyBits(&val, 0U, bits, buf, off_bits);
    return val;
}

static inline uint64_t nunavutGetU64(const void* const buf,
                                     const size_t buf_size_chars,
                                     const size_t off_bits,
                                     const size_t len_bits)
{
    ...
    NUNAVUT_ASSERT(bits <= 64U);
    NUNAVUT_ASSERT(buf != NULL);
    nunavutCopyBits(&val, 0U, bits, buf, off_bits);
    return val;
}

static inline int_fast8_t nunavutGetI8(const void* const buf,
                                  const size_t buf_size_chars,
                                  const size_t off_bits,
                                  const size_t len_bits)
{
    const size_t sat = (size_t) nunavutChooseMin(len_bits, 8U);
    int_fast8_t val = nunavutGetU8(buf, buf_size_chars, off_bits, sat);
    #if ((-1) >> 1) < 0  // Right shift is arithmetic (the most common case). So, the fast algorithm is allowed
    {
...
static inline int_fast16_t nunavutGetI16(const void* const buf,
                                    const size_t buf_size_chars,
                                    const size_t off_bits,
                                    const size_t len_bits)
{
    const size_t sat = (size_t) nunavutChooseMin(len_bits, 16U);
    int_fast16_t val = nunavutGetU16(buf, buf_size_chars, off_bits, sat);
    #if ((-1) >> 1) < 0  // Right shift is arithmetic (the most common case). So, the fast algorithm is allowed
    {
...
static inline int_fast32_t nunavutGetI32(const void* const buf,
                                    const size_t buf_size_chars,
                                    const size_t off_bits,
                                    const size_t len_bits)
{
    const size_t sat = (size_t) nunavutChooseMin(len_bits, 32U);
    int_fast32_t val = nunavutGetU32(buf, buf_size_chars, off_bits, sat);
    #if ((-1) >> 1) < 0  // Right shift is arithmetic (the most common case). So, the fast algorithm is allowed
    {
...
static inline int64_t nunavutGetI64(const void* const buf,
                                    const size_t buf_size_chars,
                                    const size_t off_bits,
                                    const size_t len_bits)
{
    const size_t sat = (size_t) nunavutChooseMin(len_bits, 64U);
    int64_t val = nunavutGetU64(buf, buf_size_chars, off_bits, sat);
    #if ((-1) >> 1) < 0  // Right shift is arithmetic (the most common case). So, the fast algorithm is allowed
    {
...
static_assert(32U == (sizeof(float) * CHAR_BIT), "Unsupported floating point model");
...
static inline uint_fast16_t nunavutFloat16Pack(const float value)
{
    ...
    if (in.bits >= f32inf.bits)
    {
        ...
        {
            out = (in.bits > f32inf.bits) ? 0x7FFFU : 0x7C00U;
        }
    }
    else
    {
...
static inline int nunavutSetF16(
    void* const buf,
    const size_t buf_size_chars,
    const size_t off_bits,
    const float value)
...
static inline int nunavutSetF32(
    void* const buf,
    const size_t buf_size_chars,
    const size_t off_bits,
    const float value)
    ...
    const size_t off_bits)
{
    ...
    union  // NOSONAR
    {
        uint_fast32_t in;
        float fl;
    } const tmp = {nunavutGetU32(buf, buf_size_chars, off_bits, 32U)};
    return tmp.fl;
}
...
static_assert(64U == (sizeof(double) * CHAR_BIT), "Unsupported floating point model");

static inline int nunavutSetF64(
    void* const buf,
    const size_t buf_size_chars,
    const size_t off_bits,
    const double value)
    ...
    const size_t off_bits)
{
    ...
    union  // NOSONAR
    {
        uint64_t in;
        double fl;
    } const tmp = {nunavutGetU64(buf, buf_size_chars, off_bits, 64U)};
    return tmp.fl;
}
